...
import { getTeachingCOurseList, getTeacherList, getCourseList, deleteRule, createTeachingCourse, updateRule } from '@/api/teachingRules'
// KD000 : end for 导入API
...
export default {
  ...
  methods: {
    ...
    getCurrentCourse() {
      getTeachingCOurseList(this.listQuery).then(response => {
        var results = []
        for (let i = 0; i < response.data.results.length; i++) {
          var dict = {}
          dict['id'] = response.data.results[i].id
          dict['teacher'] = response.data.results[i].teacher.name
          dict['course'] = response.data.results[i].course.name
          results.push(dict)
          console.log(response.data.results[i])
        }
        this.list = results
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          console.log(data)
          createTeachingCourse(data).then(() => {
