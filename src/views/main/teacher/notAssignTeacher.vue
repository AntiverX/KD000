...
import {
  getteacher,
  getnotAssignTeacher,
  createnotAssignTeacher,
  deletenotAssignTeacher,
} from '@/api/user'
// KD000 : end for 导入API
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...
    getCurrentCourse() {
      getnotAssignTeacher(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createnotAssignTeacher(this.temp).then(response => {
    ...
    handleDelete(row, index) {
      deletenotAssignTeacher(row.id).then(() => {
        this.$notify({
          title: 'Success',
