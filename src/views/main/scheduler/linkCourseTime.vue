...
import {
  getlinkCourseTime,
  createlinkCourseTime,
  deletelinkCourseTime,
} from '@/api/course'
// KD000 : end for 导入API
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...
    getCurrentCourse() {
      getlinkCourseTime(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createlinkCourseTime(this.temp).then(response => {
    ...
    handleDelete(row, index) {
      deletelinkCourseTime(row.id).then(() => {
        this.$notify({
          title: 'Success',
